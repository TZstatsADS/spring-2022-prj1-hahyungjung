...
# Data Processing - NRC Sentiment Anlaysis 

We analyze positive & negative aspects about authors' texts by using NRC sentiment anlaysis.

1. Inner join between NRC sentiments such as "positive" and "negative" and tokenized tidy words
2. Get each words' sentiment ratio and Cacluate the sentiment ratio of each author
3. Display ggplot about each sentiment ratio of each author
...
We can derive this question by analyzing, "Do philosophers use positive or negative words more?"
...
1. Inner join between NRC sentiments and tokenized tidy words
[The type of sentiments in NRC : "anger", "anticipation", "disgust", "fear", "joy", "sadness", "surprise", "trust"]
2. Get each words' sentiment value and Calculate the value of each author about sentiments
3. Display ggplot about each sentiment ratio of each school


$$ Freq\ Data = \frac{value's\ sum\ of\ each\ school}{all\ values'\ sum}$$
School Type
```{r}
...
We can derive this question by analyzing, "what is the fundamental element of many emotional aspects in philosophical expressions?"
...
First, we could make term frequency combine to calculate a term’s tf-idf (the two quantities multiplied together), the frequency of a term adjusted for how rarely it is used. It is to find the important words for the content of each document by decreasing the weight for commonly used words and increasing the weight for words that are not used very much in a collection or corpus of documents.

Second, we could discover this by finding the pairwise correlation of word frequencies within each school's texts, using the pairwise_cor() function from the widyr package. Through the correlation of each schools, we could link to each school having higher than the 0.6 value of correlation.
...
We can derive this question by analyzing, "What schools tend to be similar to each other in text contents?"
